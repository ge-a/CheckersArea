...
import CheckersAreaController from './CheckersAreaController';
...
export type TownEvents = {
  ...
  /**
   * An event that indicates that the checkers area has changed. This event is emitted after updating
   * the town controller's record of the checkers area.
   */
  checkersAreaChanged: (newCheckersArea: CheckersAreaController | undefined) => void;
  /**
...
export default class TownController extends (EventEmitter as new () => TypedEmitter<TownEvents>) {
  ...
  private _checkersArea?: CheckersAreaController;
  ...
  public get checkersArea(): CheckersAreaController | undefined {
    return this._checkersArea;
  }

  public set checkersArea(newCheckersArea: CheckersAreaController | undefined) {
    this._checkersArea = newCheckersArea;
    this.emit('checkersAreaChanged', newCheckersArea);
  }
  ...
  registerSocketListeners() {
    ...
    this._socket.on('interactableUpdate', interactable => {
      ...
      } else if (isCheckersArea(interactable)) {
        const relArea = this._checkersArea?.id == interactable.id ? this._checkersArea : null;
        if (relArea) {
          relArea.updateFrom(interactable);
        }
      }
   ...
   * @param newArea
   */
  async createCheckersArea(newArea: CheckersAreaModel) {
    await this._townsService.createCheckersArea(this.townID, this.sessionToken, newArea);
  }
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      this._socket.on('initialize', initialData => {
        ...
        initialData.interactables.forEach(eachInteractable => {
          ...
          } else if (isCheckersArea(eachInteractable)) {
            this._checkersArea = new CheckersAreaController(eachInteractable);
          }
        });
  ...
  public emitCheckersAreaUpdate(checkersArea: CheckersAreaController) {
    this._socket.emit('interactableUpdate', checkersArea.toCheckersAreaModel());
  }

  /**
   * Get the players for a specified checkers area (specified via the area controller)
   * @param checkersArea the checker area controller
   * @returns a promise wrapping the contents of a list of the two players | undefined
   */
  public async getCheckersPlayers(
    checkersArea: CheckersAreaController,
  ): Promise<(string | undefined)[]> {
    return this._townsService.getCheckersAreaPlayers(
      this.townID,
      checkersArea.id,
      this.sessionToken,
    );
  ...
  /**
   * Returns the player who won the checkers game for a specified checkers area (specified via the area controller)
   * Also posts the win and loss to the database when game is over or resign
   * @param checkersArea the checker area controller
   * @param gameOverWrapper wrapper containing the end state checker board model or possible a resigned player
   * @returns a promise wrapping the contents of a player | undefined
   */
  public async checkersWin(
    checkersArea: CheckersArea,
    gameOverWrapper: GameOverWrapper,
  ): Promise<string | undefined> {
    return this._townsService.getCheckersAreaWinner(
      this.townID,
      checkersArea.id,
      this.sessionToken,
      gameOverWrapper,
   ...
   * @returns a promise wrapping the contents of a player | undefined
   */
  public async getLeaderBoard(checkersArea: CheckersArea): Promise<GameRecord[]> {
    return this._townsService.getLeaderBoard(this.townID, checkersArea.id, this.sessionToken);
  }
  ...
  /**
   * Get the possible move paths for all pieces on a checkers board.
   * @param checkerBoardModel the checker board model
   * @returns a promise wrapping the checker's boards possible moves.
   */
  public async getCheckerBoardPossibleMoves(
    checkerBoard: CheckerTile[][],
    currentPlayer: 'red' | 'black',
  ): Promise<Pos[][][][]> {
    return this._townsService.getCheckerBoardPossibleMoves(
      this.townID,
      currentPlayer,
      this.sessionToken,
      checkerBoard,
  ...
  public async postPieceMove(
    ...
    if (
      ...
      const startPos = { row: source.y, col: source.x };
      const destPos = { row: dest.y, col: dest.x };
      const currPlayer = checkersArea.isPlayerOneTurn ? 'red' : 'black';
      const theMove: CheckerMove = {
        source: startPos,
        dest: destPos,
        currPlayer: currPlayer,
        board: checkersArea.board.board,
      };
      return this._townsService.postPieceMove(
        this.townID,
        checkersArea.id,
        this.sessionToken,
        theMove,
    ...
    throw new Error('Failed to post move');
  }
...
/**
 * A react hook to retrieve a checkers area controller.
 *
 * This function will throw an error if the checkers area controller is undefined.
 *
 * This hook relies on the TownControllerContext.
 *
 * @throws Error if there is not a valid checkers area
 */
export function useCheckersAreaController(): CheckersAreaController {
  const townController = useTownController();
  const checkersArea = townController.checkersArea;
  if (!checkersArea) {
    throw new Error(`Checkers area is not defined`);
  }
  return checkersArea;
}
