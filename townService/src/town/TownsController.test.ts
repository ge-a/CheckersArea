...
describe('TownsController integration tests', () => {
  ...
  describe('Interactables', () => {
    ...
    describe('Create Checkers Area', () => {
      it('Executes without error when creating a new checkers area', async () => {
        ...
        if (!checkersArea) {
          ...
          // Check to see that the checkers area was successfully updated
          const townEmitter = getBroadcastEmitterForTownID(testingTown.townID);
          const updateMessage = getLastEmittedEvent(townEmitter, 'interactableUpdate');
          if (isCheckersArea(updateMessage)) {
            expect(updateMessage).toEqual(newCheckersArea);
          } else {
            fail('Expected an interactableUpdate to be dispatched with the new checkers area');
          }
        }
      });
      it('Returns an error message if the town ID is invalid', async () => {
        ...
        const newCheckersArea: CheckersArea = {
          ...
          id: checkersArea.id,
          isPlayerOneTurn: true,
          preBoard: new CheckerBoard(null).precompute('red'),
        };
        await expect(
          controller.createCheckersArea(nanoid(), sessionToken, newCheckersArea),
        ).rejects.toThrow();
      });
      it('Checks for a valid session token before creating a checkers area', async () => {
        const invalidSessionToken = nanoid();
        const checkersArea = interactables.find(isCheckersArea) as CheckersArea;
        const newCheckersArea: CheckersArea = {
          board: new CheckerBoard(null),
        ...
        await expect(
          controller.createCheckersArea(testingTown.townID, invalidSessionToken, newCheckersArea),
        ).rejects.toThrow();
      });
      it('Returns an error message if addCheckersArea returns false', async () => {
        const checkersArea = interactables.find(isCheckersArea) as CheckersArea;
        checkersArea.id = nanoid();
        await expect(
          controller.createCheckersArea(testingTown.townID, sessionToken, checkersArea),
        ).rejects.toThrow();
      });
    });
  });
